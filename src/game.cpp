...
void Game::init() {
    ...
    demon.engine.data_root.find("**/keyboard_mouse").attach_new_node(mouse_watcher);
    
    // Init Mouse
    mouse.initialize(demon.engine.win, mouse_watcher);
...
void Game::create_dr3D() {	
	float size = demon.game_view_default.size;
